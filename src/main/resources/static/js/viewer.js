...
async function poll() {
  try {
    const r = await fetch("/viewer/progress?ts=" + Date.now(), {
      cache: "no-store",
    });
    ...
    const mapped = Math.min(
      100,
      Math.max(80, 80 + Math.round(serverPct * 0.2)),
    );
...
function initTxnRecordTypeFilter() {
  ...
  if (!document.getElementById(rtHelpId)) {
    ...
    help.id = rtHelpId;
    help.textContent =
      "This list is filtered by Transaction Type. Choose a transaction type first.";
  ...
  const TXN_MAP = {
    ...
    CBF: new Set(["5A", "6A", "6B", "7A", "7B", "9A"]),
    RF: new Set(["RC_31", "RC_32"]),

    NF: new Set(["NF_81", "NF_82", "NF_83", "NF_84"]),
...
(function initSessionStoragePersistence() {
  document.addEventListener("DOMContentLoaded", () => {
    ...
    try {
      sessionStorage.setItem(KEY("lastHasFile"), hasFileNow ? "true" : "false");
    } catch (_) {}
    ...
    try {
      ...
      if (txnSelect && storedTxn) {
        txnSelect.value = storedTxn;
      }
      if (filterAPI && txnSelect) {
      ...
      if (sccfInput && storedSccf) {
        sccfInput.value = storedSccf;
      }
      if (recNoInput && storedRec) {
        recNoInput.value = storedRec;
      }
    } catch (e) {}
    ...
    function persist() {
      try {
        if (txnSelect)
          sessionStorage.setItem(KEY("txnType"), txnSelect.value || "");
        if (rtSelect)
          sessionStorage.setItem(KEY("recordTypeCode"), rtSelect.value || "");
        if (sccfInput)
          sessionStorage.setItem(KEY("sccf"), sccfInput.value || "");
        if (recNoInput)
          sessionStorage.setItem(KEY("recNo"), recNoInput.value || "");
      } catch (e) {}
    }
    if (txnSelect)
      txnSelect.addEventListener("change", () => {
    ...
    if (btnClear && clearForm) {
      btnClear.addEventListener("click", () => {
        ...
        try {
          sessionStorage.setItem(KEY("lastHasFile"), "false");
        } catch (_) {}
...
function setScrollToResultsFlag() {
  try {
    sessionStorage.setItem(KEY("scrollToResults"), "true");
  } catch (_) {}
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const searchInitiallyDisabled = document
    .getElementById("btnSearch")
    ?.hasAttribute("disabled");
  const exportInitiallyDisabled = document
  ...
  if (uploadForm && pbar) {
    uploadForm.addEventListener("submit", (ev) => {
      ...
      xhr.upload.onprogress = (e) => {
        ...
        const mapped = Math.max(0, Math.min(80, Math.round(rawPct * 0.8)));
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  document.addEventListener("DOMContentLoaded", function () {
    const pager = document.getElementById("horizontalPagerData");

    if (!pager) return;

    let currentPage = parseInt(pager.dataset.currentPage || "1");
    let totalPages = parseInt(pager.dataset.totalPages || "1");

    const nextBtns = [
      document.getElementById("horizontalNextTop"),
      document.getElementById("horizontalNextBottom"),
    ];

    const prevBtns = [
      document.getElementById("horizontalPrevTop"),
      document.getElementById("horizontalPrevBottom"),
    ];
    ...
    function updateButtons() {
      ...
      const infoTop = document.getElementById("horizontalPageInfoTop");
      const infoBottom = document.getElementById("horizontalPageInfoBottom");
    ...
    function loadPage(page) {
      ...
      // preserve horizontal mode
      sessionStorage.setItem("horizontalPage", page);

      // redirect with page number
      window.location.href = `/viewer/current?page=${page}`;
    }
  ...
  document.addEventListener("input", function (event) {
    ...
    if (!el || !el.classList.contains("editable-field")) {
      return;
    }
...
function updateSaveState() {
  ...
  const anyInvalid = inputs.some(
    (i) => !i.readOnly && i.classList.contains("is-invalid"),
  );
...
document.querySelectorAll(".editable-field").forEach(function (el) {
  ...
  document.body.addEventListener(
    ...
    (ev) => {
      ...
      if (!el || !el.classList || !el.classList.contains("editable-field"))
        return;
      if (ev.key.length !== 1) return; //allow control key
      if (/[-?]/.test(ev.key)) {
        ev.preventDefault();
      }
    },
  ...
  if (form) {
    form.addEventListener("submit", (ev) => {
      ...
      if (!ok) {
        ...
        alert("Please fix validation error before saving.");
      }
    });
  }

  //Revalidating while typing so save stays enabled when fields are valid
  ...
  //keep API Compatibility with older code that may cal this function
  window.restrictByFirstFieldType = function () {
